                        ...
                        <form id="quote_form">
                            <div class="row">
                                ...
                                <div class="mb-3 col-lg-9">
                                    <label for="exampleFormControlInput1" class="form-label text-capitalize">drop off
                                        at</label>
                                    <input type="text" class="form-control" id="geoDropOff2"
                                           placeholder="Type Your Address" required>
                                </div>
...
<script>
    ...
    function initAutocomplete() {
        ...
        dropOff1 = new google.maps.places.Autocomplete(
        ...
        dropOff2 = new google.maps.places.Autocomplete(
            document.getElementById('geoDropOff2'),
            {
                types: ['address'],
                componentRestrictions: {'country': ['uk']},
                fields: ['place_id', 'geometry', 'address_components']
        ...
        autoPickup.addListener('place_changed', function (){
            onPlacePickup(autoPickup);
        });
        dropOff1.addListener('place_changed', function (){
            onPlaceDropOff(dropOff1);
        });
        dropOff2.addListener('place_changed', function (){
            onPlaceDropOff(dropOff2);
        });
    }

    function onPlacePickup(res) {
    ...
    function onPlaceDropOff(res) {
        let place2 = res.getPlace();
        // get postcode
        ...
        if (!place2.geometry) {
            ...
            // add to array
            deliveryArray.push(';'+ geoDropOffLong, geoDropOffLat)
            //console.log(deliveryArray.toString())
            endRoute(deliveryArray);
        }
    }
    ...
    async function getRoute() {
        ...
        const query = await fetch(
            `https://api.mapbox.com/directions/v5/mapbox/driving/${geoPickupLong},${geoPickupLat}${deliveryArray}?steps=true&geometries=geojson&access_token=${mapboxgl.accessToken}`,
            {method: 'GET'}
        );
        //console.log(query)
        ...
        console.log(data.distance * 0.00062137)
        miles = data.distance * 0.00062137
    ...
    function endRoute(finish) {
        ...
        // add long and lat to finish array then all the getRoute func pass finish
        getRoute();
...
<script async
        src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBotrQx2CHtqHAmNE-EbicRVhSg2y5UqeY&libraries=places&callback=initAutocomplete">
</script>
